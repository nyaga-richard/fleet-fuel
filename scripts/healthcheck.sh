...
check "postgres container"     bash -c "[ \"\$(docker inspect --format '{{.State.Health.Status}}' \"\$(compose ps -q postgres 2>/dev/null)\" 2>/dev/null)\" = 'healthy' ]"
check "postgres accepts conns" compose exec -T postgres pg_isready -U "$(env_value POSTGRES_USER fleetfuel)" -d "$(env_value POSTGRES_DB fleetfuel)"
